...
* 사용자 및 내부 용량 인식 테스트
  1. **1 Set당 20번의 거리 측정 인식 테스트를 10초 간격으로 수행**  
  2. **1 Set가 종료되면 2분간 대기하고 다음 Set 반복 진행**

* 안전성 테스트
  1. **서보 모터의 내구성 확인을 위해 배터리를 교체하며 1000번의 반복 수행**
  2. **서보 모터의 작동 여부를 확인하며, 반복 수행에 대한 적합성을 확인**
  3. **LED가 On 상태에서부터 시작하여 10분 간격으로 측정하고, 최대 지속가능한 시간을 측정을 위해 On 상태를 유지**

### 5.2 테스트 결과

* 사용자 및 내부 용량 인식 테스트 결과
  1. 사용자 인식 테스트 에서는 5 Set와 10 Set에 수행 하던 도중 배터리의 방전으로 작동 불가
  2. 서보 모터와 LED 기능 구동 시 하나의 기능이 작동하지 않아 저전압 특성으로 파악

* 반복 수행 안전성 테스트 결과
  1. 서보 모터와 LED의 반복 수행 테스트에서 별도의 문제는 발생하지 않았지만 배터리의 수명이 한계에 도달함 으로써 이를 보안할 방법을 모색
